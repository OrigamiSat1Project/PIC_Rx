...
void main(void) {
    ...
    while(1){
        ...
        if (strcmp(dData,EPS_kill) == 0){
            ...
            Reset_EPS();
            __delay_ms(5000);
            //ˆÈ‰º‚Ì”Žš‚Í‰ŠúÝ’èŽž‚Æ•Ï‰»‚µ‚Ä‚¢‚é‚½‚ß‚à‚¤ˆê“x’è‹`
        ...
        if(dData[0]=='n'){      //NanoMind
            ...
            if(dData[2] == 'r'){        // PIC_RX
                ...
                case 'E':
                    ...
                    Reset_EPS();
                    __delay_ms(5000);
                    //ä»¥ä¸‹ã®æ•°å­—ã¯åˆæœŸè¨­å®šæ™‚ã¨å¤‰åŒ–ã—ã¦ã„ã‚‹ãŸã‚ã‚‚ã†ä¸€åº¦å®šç¾©
                    //æœ¬æ¥ãªã‚‰å¤‰åŒ–ã™ã‚‹æ–‡å­—åˆ—ã‚’ä»–ã«ç”¨æ„ã—ãŸã»ã†ãŒè‰¯ã„ã‹ã‚‚ã—ã‚Œãªã„
                    int FMTX_Nprg[5]     =   {8,7,3,0,0};   // Nprg = 87300 = Ftx / 0.05 [436.500MHz]
                    int CWTX_Nprg[5]     =   {0,1,7,4,7};   // Nprg = 1747(* see 301ACWPLL-20080520.pdf *) [436.750MHz]
                    int FMRX_Nprg[5]     =   {2,4,8,8,7};   // Nprg = 24887 = (Frx - 21.4) / 0.05 [145.835MHz]
